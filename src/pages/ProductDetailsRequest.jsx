import React, { useEffect, useState } from 'react';
import { useLocation, useNavigate, useOutletContext, useParams, useSearchParams } from 'react-router-dom';
import { getProducts } from '../utils/localStorageUtils';
import './ProductDetails.css';
import NoInfo from '../components/NoInfo';
import Loading from '../components/Loading';
import { imageUrlApi } from '../api/axiosConfig';
import { useMutation } from '@tanstack/react-query';
import { changeOfferState, shopProposePrice } from '../api/queries';

const ProductDetailsRequest = () => {

  const location = useLocation()
  const state = location.state.data

  const [price, setPrice] = useState(state.precio)

  const navigate = useNavigate();

  const {  mutate: mutateReject, isPending: isPendingReject } = useMutation({
      mutationFn: () => changeOfferState(state.idoferta, 'rechazada'),
      onSuccess: () => navigate(-1)
  })

  const { mutate: mutateAccept, isPending: isPendingAccept } = useMutation({
    mutationFn: () => shopProposePrice(state.idoferta, price),
    onSuccess: () => navigate(-1)
  })

  function handleAccept() {
    mutateAccept()
  }

  function handleReject() {
    mutateReject();
  }

  if (isPendingReject || isPendingAccept){
    return <Loading />
  }


  return (
    <div className="flex flex-col items-center py-8">
      <h2 className=' font-medium text-2xl text-gray-500'>Gestionar solicitud</h2>
      <div className="flex gap-x-16 w-full pl-60 pr-48 my-10">
        <div className="w-2/5">
          <img className=" w-full h-auto" src={`${imageUrlApi}/${state.imagen}`} alt={state.nombre} />
        </div>
        <div className="flex flex-col pl-5 pr-20 pt-5 text-left flex-1">
          <div className="border-b border-b-gray-200 pb-2 mb-2">
            <h2 className=" font-semibold text-2xl">{state.nombre}</h2>
            <p className='mt-1 text-base font-light text-gray-500'>Categoría: {state.categoria}</p>
          </div>

          <p className='mt-8 pb-8 font-medium text-3xl border-b border-b-gray-200'>$ {state.precio}</p>

          {
            state.estado === "pendiente_tienda" ?

            <div className="">
              <div className="flex gap-x-4 items-center justify-center mt-3">
                <label className='text-gray-500'>Precio a proponer</label>
                <input
                  type="number"
                  className='text-gray-500 rounded border border-gray-300 px-3 py-1 my-2'
                  value={price}
                  onChange={(e) => setPrice(e.target.value)}
                />
              </div>
              <div className="flex items-center gap-x-5">
                <button onClick={handleAccept} className='mt-7 rounded-full bg-firstColor text-white hover:opacity-80 py-1 px-6 self-center'>
                  Aceptar producto
                </button>
                <button onClick={handleReject} className='mt-7 rounded-full bg-firstColor text-white hover:opacity-80 py-1 px-6 self-center'>
                  Rechazar producto
                </button>
              </div>
            </div>
            :
            <p className='my-4'>En espera de que el cliente confirme la operación</p>
          }

          <div className='mt-8 border border-gray-200 p-3 rounded'>
            <h4 className='pb-2 font-semibold text-lg w-full border-b border-b-gray-200'>Descripción</h4>
            <p className='mt-4 text-gray-700'>{state.descripcion}</p>
          </div>
        </div>
      </div>
    </div>
  );
}

export default ProductDetailsRequest;
